...
window.addEventListener('load', () => {
  ...
  submitButton.addEventListener('click', () => {
    ...
    let selectedFileType = selectedFile.name
    const allowedExtensions = new RegExp('(.*?)\\.(csv)$');
    ...
    //TOO MUCH MESS IN HERE!!!
    // MAKE FUNCTIONS FOR THESE GRAPHS
    ...
    uploadFile('/uploadFile', data).then((response) => {
      ...
      let promises = dataResponse.map((item) => {
    ...
    window.sendToDb.addEventListener('click', () => {
      let data = {
        filepath: uploadedFilePath
      }

      //dont sent the data all over again, send the file to reuse
      ...
      fetch('http://localhost:3000/sendTodb', option) //use more meaningful names for routes 'was send before' does not explain enough...
...
function showSpinner() {
  window.waitspinner.classList.remove('hidden')
  window.map.classList.add('blur')
}

function hideSpinner() {
  window.waitspinner.classList.add('hidden')
  window.map.classList.remove('blur')
